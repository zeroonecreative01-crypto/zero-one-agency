...
type Field = { label: string; type: Override['type']; target: string; hint?: string };
  ...
  return <section className="mt-10">
    <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-[#F7F5F0]/45">اختار العنصر من القائمة بدل ما تدور على النص يدويًا. لو العنصر موجود بالفعل هيتفتح تعديلُه مباشرة، وبعدها Save changes لتطبيقه على الموقع.</p>
      </div>
    ...
    <div className="border border-[#F7F5F0]/10 bg-[#F7F5F0]/[0.02] p-5 md:p-7">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {fields.map((field) => {
          const active = existingTargets.has(`${field.type}:${field.target}`);
          return <button key={`${field.type}:${field.target}`} type="button" onClick={() => chooseField(field)} className={`rounded-xl border p-4 text-left transition ${active ? 'border-[#F14A0B]/50 bg-[#F14A0B]/10' : 'border-[#F7F5F0]/10 bg-[#F7F5F0]/[0.02] hover:border-[#F14A0B]/40'}`}><div className="flex items-center justify-between gap-2"><span className="text-sm font-semibold">{field.label}</span><span className="text-[9px] uppercase tracking-widest text-[#F14A0B]">{field.type}</span></div><p className="mt-2 truncate text-[11px] text-[#F7F5F0]/35">{field.target}</p></button>;
        })}
      </div>

      <div className="mt-6 border-t border-[#F7F5F0]/10 pt-6">
        <div className="grid gap-3 md:grid-cols-[140px_1fr_1fr_auto] md:items-end">
