...
export function AdminRoute() {
  ...
  if (loading) {
    return <AuthShell><div className="min-h-screen flex items-center justify-center text-sm uppercase tracking-[0.2em] text-[#F7F5F0]/50">Checking session...</div></AuthShell>;
  }

  if (!configured) {
    return <AuthShell><div className="min-h-screen flex items-center justify-center px-6"><div className="w-full max-w-lg border border-[#F7F5F0]/10 p-8 md:p-12"><p className="text-[#F14A0B] text-xs font-bold uppercase tracking-[0.2em] mb-4">Authentication Setup Required</p><h1 className="text-3xl font-bold tracking-tight mb-4">Supabase is not configured.</h1><p className="text-[#F7F5F0]/60 leading-relaxed">Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to the deployment environment, then redeploy.</p></div></div></AuthShell>;
  }
