...
import { formatMarketPrice, getMarket, getStoredMarket, MARKETS, PACKAGE_PRICES, setStoredMarket, type MarketCode } from '../lib/pricingMatrix';
...
function renderPricing(section: HTMLElement, packages: PricingPackage[], market: MarketCode) {
  ...
  const render = (direction: 'next' | 'prev' | 'direct' = 'direct') => {
    ...
    grid.innerHTML = `<div class="zero-one-pricing-carousel" role="region" aria-label="Pricing packages" aria-live="polite"><button type="button" class="zero-one-pricing-carousel__arrow zero-one-pricing-carousel__arrow--prev" data-pricing-prev aria-label="Previous package">←</button><div class="zero-one-pricing-carousel__viewport"><div class="zero-one-pricing-carousel__card" data-direction="${direction}">${cardMarkup(pkg, market)}</div></div><button type="button" class="zero-one-pricing-carousel__arrow zero-one-pricing-carousel__arrow--next" data-pricing-next aria-label="Next package">→</button></div><div class="zero-one-pricing-carousel__footer"><div class="zero-one-pricing-carousel__dots" role="tablist" aria-label="Select pricing package">${sorted.map((item, index) => `<button type="button" role="tab" aria-selected="${index === activeIndex}" aria-label="View ${esc(item.name)} package" class="zero-one-pricing-carousel__dot${index === activeIndex ? ' is-active' : ''}" data-pricing-index="${index}"><span>${String(index + 1).padStart(2, '0')}</span></button>`).join('')}</div><div class="zero-one-pricing-carousel__counter"><strong>${String(activeIndex + 1).padStart(2, '0')}</strong><span>/ ${String(sorted.length).padStart(2, '0')}</span></div></div>`;
...
function setupMarketSelector(section: HTMLElement, packages: PricingPackage[], initialMarket: MarketCode) {
  ...
  selector.querySelectorAll<HTMLButtonElement>('[data-market-code]').forEach((button) => button.addEventListener('click', () => {
    ...
    selector.querySelectorAll('[data-market-code]').forEach((item) => { item.classList.toggle('is-active', (item as HTMLElement).dataset.marketCode === market); item.setAttribute('aria-selected', String((item as HTMLElement).dataset.marketCode === market)); });
    renderPricing(section, packages, market);
  }));
  renderPricing(section, packages, market);
}
...
export default function RemotePricingSync() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      if (attempts++ < 40) window.setTimeout(() => {
        const current = getStoredMarket();
        if (section.querySelector('[data-market-selector]')) {
          const grid = section.querySelector('.zero-one-pricing__grid');
          if (grid && !grid.querySelector('.zero-one-pricing-carousel')) renderPricing(section, packages, current);
        }
      }, 100);
    };
    run();
  ...
  useEffect(() => {
    const onMarketChange = () => {
      if (!packages) return;
      const section = document.getElementById('zero-one-pricing');
      if (section) renderPricing(section, packages, getStoredMarket());
    };
