...
import { ArrowRight, HelpCircle, Mail, MessageCircle, Send, X } from 'lucide-react';
...
function useClientConversation(token: string | null) {
  ...
  useEffect(() => {
    ...
    if (!client || !token) { setData(null); setLoading(false); return; }
    let mounted = true;
    setLoading(true);
    client.rpc('get_support_conversation', { p_visitor_token: token }).then(({ data: result, error }) => {
      if (!mounted) return;
  ...
  useEffect(() => {
    ...
    const channel = client.channel(`support:${token}`).on('broadcast', { event: 'INSERT' }, (payload) => {
...
export function SupportCenter() {
  ...
  const startConversation = async (event: React.FormEvent) => {
    ...
    const { data: result, error: rpcError } = await client.rpc('create_support_conversation', { p_name: form.name, p_email: form.email, p_phone: form.phone, p_country: form.country, p_language: form.language, p_support_type: form.supportType, p_details: form.details, p_message: form.message });
    setSubmitting(false);
  ...
  const sendClientMessage = async (event?: React.FormEvent) => {
    event?.preventDefault();
    const client = supabase;
    if (!client || !token || !message.trim()) return;
    const text = message.trim(); setMessage('');
    const { error: rpcError } = await client.rpc('send_support_message', { p_visitor_token: token, p_message: text });
  ...
  return <>
    {open && <div className="fixed inset-0 z-[100] flex items-end justify-end p-4 sm:p-6 md:p-8 pointer-events-none"><div className="pointer-events-auto flex h-[min(720px,calc(100vh-32px))] w-full max-w-[440px] flex-col overflow-hidden rounded-[28px] border border-[#F7F5F0]/10 bg-[#111111] shadow-[0_30px_100px_rgba(0,0,0,.55)]">
      ...
      </div><button disabled={submitting} className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-[#F14A0B] px-5 py-3.5 text-sm font-semibold text-[#111111] disabled:opacity-50">{submitting ? 'Starting...' : 'Start Conversation'} <ArrowRight size={15} /></button><a href={`mailto:${SUPPORT_EMAIL}`} className="mt-4 flex items-center justify-center gap-2 text-xs text-[#F7F5F0]/40"><Mail size={13} /> {SUPPORT_EMAIL}</a></form> : loading ? <div className="flex flex-1 items-center justify-center text-xs uppercase tracking-[0.2em] text-[#F7F5F0]/35">Loading conversation...</div> : conversationData ? <><div ref={messagesRef} className="flex-1 space-y-3 overflow-y-auto p-5"><div className="mb-5 rounded-2xl border border-[#F7F5F0]/8 bg-[#F7F5F0]/[0.03] p-4"><span className="text-[9px] font-bold uppercase tracking-[0.18em] text-[#F14A0B]">{conversationData.client.language} · {conversationData.conversation.support_type}</span><p className="mt-2 text-sm text-[#F7F5F0]/65">Hi {conversationData.client.name.split(' ')[0]}, we’re here to help.</p></div>{conversationData.messages.map((item) => <MessageBubble key={item.id} item={item} />)}</div><form onSubmit={sendClientMessage} className="border-t border-[#F7F5F0]/10 p-4"><div className="flex items-end gap-2 rounded-2xl border border-[#F7F5F0]/10 bg-[#F7F5F0]/[0.03] p-2"><textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); e.currentTarget.form?.requestSubmit(); } }} rows={1} placeholder="Write a message..." className="min-h-10 flex-1 resize-none bg-transparent px-2 py-2 text-sm outline-none" /><button disabled={!message.trim()} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#F14A0B] text-[#111111] disabled:opacity-30"><Send size={15} /></button></div><div className="mt-3 flex items-center justify-between"><span className="text-[9px] text-[#F7F5F0]/25">Replies appear here in real time.</span><button type="button" onClick={reset} className="text-[9px] font-semibold uppercase tracking-widest text-[#F7F5F0]/35">Start over</button></div></form></> : null}</div></div>}
...
export function SupportPanel() {
  ...
  const loadConversations = async () => {
    const client = supabase;
    if (!client) return;
    const { data } = await client.from('support_conversations').select('*, support_clients(*)').order('last_message_at', { ascending: false });
    setConversations(data || []); if (!selectedId && data?.[0]?.id) setSelectedId(data[0].id); setLoading(false);
  };
  const loadMessages = async (id: string) => {
    const client = supabase;
    if (!client) return;
    const { data } = await client.from('support_messages').select('id,conversation_id,sender_type,message,created_at,read_at').eq('conversation_id', id).order('created_at', { ascending: true });
    setMessages((data || []) as Message[]);
    await client.from('support_messages').update({ read_at: new Date().toISOString() }).eq('conversation_id', id).eq('sender_type', 'client').is('read_at', null);
  };
  ...
  useEffect(() => {
    const client = supabase;
    if (!client) return;
    const channel = client.channel('support-admin-inbox').on('postgres_changes', { event: '*', schema: 'public', table: 'support_messages' }, (payload) => {
  ...
  const sendReply = async (event?: React.FormEvent) => {
    event?.preventDefault();
    const client = supabase;
    if (!client || !selectedId || !reply.trim()) return;
    const text = reply.trim(); setReply(''); setSending(true);
    const { error } = await client.from('support_messages').insert({ conversation_id: selectedId, sender_type: 'admin', message: text });
    setSending(false); if (error) setReply(text);
  };
  const updateConversation = async (patch: Record<string, string>) => { const client = supabase; if (!client || !selectedId) return; await client.from('support_conversations').update(patch).eq('id', selectedId); await loadConversations(); };
